...
/* Post Thumbnail Support
  /*----------------------------------------------------------------------------------- */
  function blackbird_post_thumbnail(){
    add_theme_support('post-thumbnails');
}
add_action('after_setup_theme', 'blackbird_post_thumbnail');
 ...
 * @param type $ih 
 */
function blackbird_get_thumbnail($iw, $ih) {
    $permalink = get_permalink($id);
    $thumb = get_post_thumbnail_id();
    $image = blackbird_thumbnail_resize($thumb, '', $iw, $ih, true, 90);
...
//This is required to set to Null
    ...
    $img_path = blackbird_image_resize($img_source, $w, $h);
...
//Adding Custom Field
add_post_meta(1, 'BannerImage', true);
?>
